...

{
    ...
    model->setRootPath("");
    // 设置定时器定期检测死锁（比如每5秒检测一次）
    deadlockTimer = new QTimer(this);
    connect(deadlockTimer, &QTimer::timeout, this, &MainWindow::checkDeadlocks);
    deadlockTimer->start(5000); // 5000毫秒 = 5秒
...
void MainWindow::checkDeadlocks()
{
    if (g_transactionManager) {
        bool deadlockDetected = g_transactionManager->detectAndHandleDeadlock();
        if (deadlockDetected) {
            // 如果检测到并处理了死锁，向用户输出信息
            Utils::print("检测到死锁，已中止一个事务以解决问题");

            // 如果被终止的事务是当前事务，重置当前事务ID
            if (g_currentTransactionId != -1 &&
                !g_transactionManager->getTransaction(g_currentTransactionId)) {
                Utils::print("警告：当前事务已被死锁检测机制终止");
