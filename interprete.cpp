...
void interpret(const QString &command)
{
    ...
    if (op == "HELP") {
        ...
        if (g_currentTransactionId != -1) {
            currentTransaction = g_transactionManager->getTransaction(g_currentTransactionId);
        }

        // 如果在事务中，先获取排他锁
        if (currentTransaction) {
            if (!currentTransaction->write(tokens[2])) {
                Utils::print("错误: 无法获取数据库 " + tokens[2] + " 的排他锁");
                return;
            }
            Utils::print("获取数据库 " + tokens[2] + " 的排他锁成功");
        }
        ...
        if (currentTransaction) {
            if (!currentTransaction->write(tokens[2])) {
                Utils::print("错误: 无法获取表 " + tokens[2] + " 的排他锁");
                return;
            }
            Utils::print("获取表 " + tokens[2] + " 的排他锁成功");
        }
      ...
      //暂时不加锁
        ...
        if (currentTransaction) {
            if (!currentTransaction->write(tokens[2])) {
                Utils::print("错误: 无法获取索引 " + tokens[2] + " 的排他锁");
                return;
            }
            Utils::print("获取索引 " + tokens[2] + " 的排他锁成功");
        }
    ...
    // 处理事务命令
        ...
        // 开始事务
        if (g_currentTransactionId != -1) {
            Utils::print("错误: 已经有一个活动事务");
            return;
        }

        g_currentTransactionId = g_transactionManager->beginTransaction();
        Utils::print("事务已开始, 事务ID: " + QString::number(g_currentTransactionId));
        return;
    }
    else if (op == "COMMIT") {
        if (!Auth::checkPermission(Auth::ADMIN)) {
        ...
        // 提交事务
        if (g_currentTransactionId == -1) {
            Utils::print("错误: 没有活动事务可提交");
            return;
        }

        bool success = g_transactionManager->commitTransaction(g_currentTransactionId);
        if (success) {
            Utils::print("事务 " + QString::number(g_currentTransactionId) + " 已提交");
            g_currentTransactionId = -1;
        } else {
            Utils::print("错误: 无法提交事务 " + QString::number(g_currentTransactionId));
        }
        return;
    }
    else if (op == "ROLLBACK") {
        ...
        // 回滚事务
        if (g_currentTransactionId == -1) {
            Utils::print("错误: 没有活动事务可回滚");
            return;
        }

        g_transactionManager->rollbackTransaction(g_currentTransactionId);
        Utils::print("事务 " + QString::number(g_currentTransactionId) + " 已回滚");
        g_currentTransactionId = -1;
        return;
    }else {
        QTextStream cout(stdout);
